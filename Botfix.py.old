...
def message_received(update, context):
    ...
    if chat_id in allowed_chat_ids:
        ...
        if command == '/who':
            ...
            thread = threading.Thread(target=blink, args=(chat_id,))
            thread.start()
            context.bot.send_message(chat_id, 'LED blinking started')
        elif command == '/blinkoff':
            flag = False
            context.bot.send_message(chat_id, 'LED blinking stopped')
        elif command == '/fanon':
            ...
            run = subprocess.run(
                ["""vmstat 1 2 | tail -1 | awk '{print $13}'"""], shell=True, capture_output=True)
            context.bot.sendMessage(
            ...
            run = subprocess.run([command
                                 + """ -c 3 | awk -F ' ' '{print substr($7, 1, length($1) 0)}' | sed -e 's/\(data.\|packet\)//'"""
                                  ], shell=True, capture_output=True)
            context.bot.sendMessage(chat_id, 'Ping'
                                    + run.stdout.decode('utf-8'))
            print(run.stdout)
        elif re.search('sudo', command):
            run = subprocess.run([command], shell=True,
                                 capture_output=True)
            context.bot.sendMessage(chat_id, 'Sudo '
                                    + run.stdout.decode('utf-8'))
            print(run.stdout)
        elif command == '/Fuck':
            ...
            context.bot.sendMessage(chat_id,
                                    '/start /shutdown /startminer /stopminer /fanon /fanoff /ledon /ledoff /blinkon /blinkoff /eth0 /wlan0 /ping /sudo /help /htop /sync /model /uptime /where /who /hd /hdex /volts /speed /cpughz /cpu /wanip /lanip /temp /reboot '
                                    )
        elif command == '/start':
            context.bot.sendMessage(chat_id,
                                    '/start /shutdown /startminer /stopminer /fanon /fanoff /ledon /ledoff /blinkon /blinkoff /eth0 /wlan0 /ping /sudo /help /htop /sync /model /uptime /where /who /hd /hdex /volts /speed /cpughz /cpu /wanip /lanip /temp /reboot ')
